    ...
    std::vector<cv::DMatch>& matches)
{
    ...
    const cv::Mat ref_T_f =
        ref_frame->behavior()->worldInCameraT() *
        frame->behavior()->cameraInWorldT();
    // get the transform from last frame to this frame, don't copy
    const auto& ref_map_points = ref_frame->behavior()->obsMapPoints();
    auto conv_func = frame->behavior()->worldToCamera<float>;
    match(
        frame->frame(),
        ref_frame->frame(),
        conv_func,
...
void BruteForceWithProjectionMatcher::match(
    ...
    std::vector<cv::DMatch>& matches)
{
    // this stores current pose information into thread unchangeable pose
    // variables which are then used by worldToCameraStatic().
    frame->behavior()->updateWorldInCamStatic();
    ...
    std::vector<cv::DMatch>& matches)
{
        ...
        size_t ref_idx = 0; ref_idx < ref_frame->nFeaturesUndist(); ++ref_idx)
    {
        ...
        if (mp) { // every feature may not have 3d correspondence
            ...
            auto point_3d_cam = conversion_func(point_3d_world);
    ...
    applyRotationConstraint(rot_hist, matched);
...
{
    auto conv_func = frame->behavior()->worldToCamera<float>;
    match(frame->frame(), conv_func, points_3d, matches);
}
        ...
        size_t ref_idx = 0; ref_idx < points_3d.size(); ++ref_idx)
    {
        ...
        if (mp) { // may be not every feature has a 3d correspondence
            if (compute_track_info_) {
                ...
                if (camera_view_func(mp, track_results, 0.5)) {
...
{
    // get the transform from last frame to this frame, don't copy
    const auto& map_points = frame->behavior()->obsMapPoints();
    const auto& ref_map_points = ref_frame->behavior()->obsMapPoints();
    match(
        frame->frame(),
        ref_frame->frame(),
    ...
    std::vector<cv::DMatch>& matches)
{
    ...
    // create matches
    // matched.size() = key_points.size(), just for clarity
    for (size_t idx = 0; idx < key_points.size(); ++idx) {
        if (matched[idx] > 0) { // matches from frame to reference frame
            matches.push_back(
                cv::DMatch( // point in frame -> point in ref_frame
                    idx, matched[idx], static_cast<float>(feature_dists[idx])));
        }
    }
}
    ...
    std::vector<cv::DMatch>& matches)
{
    ...
    frame->behavior()->computeFundamentalMat(
        fundamental_mat, ref_frame->behavior());
    match(
        frame->frame(),
        ref_frame->frame(),
        fundamental_mat,
...
{
    const auto& matcher =
        static_pointer_cast<EpipolarConstraintWithBowMatcher>(
            matcher_[EPIPOLAR_CONSTRAINT]);
    matcher->check_orientation_ = check_orientation;
    matcher->match(frame, ref_frame, matches);
    if (filter_matches)
