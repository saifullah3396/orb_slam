...
void Tracker::updateLocalMapKeyFrames()
{
    ...
    for(const auto& kf: key_frames_local_map_)
    {
        ...
        for(const auto& child: kf->childs()) {
            // if it is not bad frame and is not already in local map
            if (!child->isBad() && !child->isInLocalMapOf(current_frame_->id())) {
                ...
                break; // why break? dunno yet. maybe to limit the number of childs to 1?
            }
        }
...
bool Tracker::projectLocalPoints()
{
    ...
    // variables which are then used by isInCameraViewStatic().
    current_frame_->updateWorldInCamStatic();
    ...
    for (const auto& mp: map_points_local_map_) {
        ...
        mp->resetTrackProperties();
        if (current_frame_->isInCameraViewStatic(mp, track_results, 0.5)) {
        ...
        if (!depth_indices.empty()) {
            ...
            for (const auto& depth_idx: depth_indices) {
                ...
                if (create_new_point) {
                    ...
                    auto world_pos =
                        cv::Mat(
                            current_frame_->frameToWorldStatic<float, float>(
